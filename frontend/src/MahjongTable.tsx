...
const MahjongTable = () => {
    ...
    useEffect(() => {
        ...
        // Zoom limits
        controls.minDistance = 8;   // Can't zoom too close
        controls.maxDistance = 30;  // Can't zoom too far

        // CRITICAL: Vertical angle limits (prevents seeing other players' tiles!)
        controls.minPolarAngle = Math.PI / 6;   // 30° from top (can't look straight down)
        controls.maxPolarAngle = Math.PI / 2.5; // ~72° (can't go horizontal - would see opponents)

        // Full 360° rotation allowed
        controls.enableRotate = true;

        // Disable panning (keeps focus on table)
        ...
        const onClick = (event: MouseEvent) => {
            ...
            if (intersects.length > 0) {
                ...
                if (tile && playerTilesRef.current.includes(tile as THREE.Group)) {
                    ...
                    console.log('playerTilesRef.current', playerTilesRef.current)
                    // Discard the tile
                    console.log(tile.userData);
                    setSelectedTile(tile.userData)
    ...
    const getTileImage = (number: number, type: string) => {
        ...
        return TILE_IMAGES[key] || FLOWERS; // fallback to CHARS_1 if not found
    };
    ...
    return (
        <div className=" w-screen h-screen m-0 p-0 overflow-hidden bg-gray-900">
            ...
            <div className="h-80 flex flex-col gap-10 max-w-96" style={{
                ...
                <section> <h2 style={{ fontSize: '1.25rem', fontWeight: 'bold', marginBottom: '0.5rem' }}>Mahjong Table</h2>
                    ...
                    <p style={{ fontSize: '0.75rem', color: '#d1d5db' }}>Discarded: {discardedTilesRef.current.length}</p></section>
            ...
            <div style={{
                ...
                <button style={{
                    ...
                    fontWeight: 'bold',
                    color: 'white',
                    backgroundColor: '#984fb9ff',
                    border: 'none',
                    borderRadius: '0.5rem',
                    cursor: 'pointer',
                    boxShadow: '0 4px 6px rgba(0, 0, 0, 0.3)',
                    transition: 'all 0.2s'
                }} onClick={handleSortTiles}>
                    Sort tiles
                </button>     <button style={{
                    padding: '1rem 2rem',
                    fontSize: '1rem',
                    fontWeight: 'bold',
                    color: 'white',
                    backgroundColor: '#c29810ff',
                    border: 'none',
                    borderRadius: '0.5rem',
                    cursor: 'pointer',
                    boxShadow: '0 4px 6px rgba(0, 0, 0, 0.3)',
                    transition: 'all 0.2s'
                }} onClick={handleExchangeFlowers}>
                    Exchange flowers
                </button>
                <button
